...
import '../providers/vector_store_provider.dart';
...
class SyncService extends StateNotifier<SyncState> {
  ...
  SyncService(this._driveService, this._userEmail, this._ref) : super(SyncState()) {
    ...
    // Start periodic sync every 1 minute for near real-time updates
    _autoSyncTimer = Timer.periodic(const Duration(minutes: 1), (timer) {
      if (_userEmail != null) syncAll();
    });
  }
  ...
  Future<void> performSync() async {
    ...
    try {
      ...
      if (await dbFile.exists()) {
        ...
        final matches = driveFiles.where((f) => f.name != null && f.name == dbBaseName);
        final existingDbId = matches.isNotEmpty ? matches.first.id : null;

        await _driveService.uploadFile(
          File(snapshotPath), 
          driveFileId: existingDbId,
        );
      ...
      for (final boxName in hiveBoxes) {
        ...
        if (await boxFile.exists()) {
          final driveFiles = await _driveService.listAppDataFiles();
          final matches = driveFiles.where((f) => f.name != null && f.name == boxName);
          final existingId = matches.isNotEmpty ? matches.first.id : null;
          await _driveService.uploadFile(boxFile, driveFileId: existingId);
        }
      }
      ...
      if (await libDir.exists()) {
        ...
        for (final pdf in pdfFiles) {
          ...
          final matches = driveFiles.where((f) => f.name != null && f.name == fileName);
  ...
  Future<void> performRestore() async {
    ...
    try {
      ...
      // 1. Close Database before replacing file
      try {
        await _ref.read(vectorStoreProvider).close();
      } catch (e) {
        debugPrint('DB Close Error (Expected if not init): $e');
      }
      ...
      for (final driveFile in driveFiles) {
        ...
        } else if (driveFile.name!.endsWith('.hive')) {
          final boxName = driveFile.name!.replaceAll('.hive', '');
          // Close box if open
      ...
      // 3. Status success will be listened by other providers to refresh themselves
...
final syncProvider = StateNotifierProvider<SyncService, SyncState>((ref) {
  ...
  return SyncService(driveService, email, ref);
});
