...
class VectorStoreService {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        await db.execute('CREATE INDEX IF NOT EXISTS idx_documents_hash ON documents(md5_hash)');
      },
      onOpen: (db) async {
  ...
  Future<void> _migrateDatabase(Database db) async {
    ...
    for (final column in columns) {
      await _addColumnIfNotExists(db, 'documents', column, 'TEXT');
    }
  }
  ...
  Future<void> _addColumnIfNotExists(
    ...
    try {
      ...
      if (!columnExists) {
        ...
        print('✅ Database: Kolom $column berhasil ditambahkan ke tabel $table.');
      }
    } catch (e) {
      print('❌ Database Migration Error ($column): $e');
    }
  }
