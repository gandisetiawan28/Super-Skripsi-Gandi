...
import '../prompts/metadata_extraction_prompt.dart';
...
class AiExtractionService {
  ...
  Future<DocumentMetadata?> _tryLocalhost(String excerpt, {String? model, Function(String)? onLog}) async {
    ...
    final urlInput = keyToUse['key']!.replaceAll('localhost', '127.0.0.1');
    ...
    String baseUrl = urlInput.startsWith('http') ? urlInput.replaceAll(RegExp(r'\/+$'), '') : 'http://127.0.0.1:3000';
    ...
    final bool isGeminiFlow = baseUrl.contains(':3000') || baseUrl.contains('127.0.0.1');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return _parseResponse(content as String);
      } else {
  ...
  Future<String> _customLocalhost(String systemPrompt, String userText, {String? model, bool isJson = true, Function(String)? onLog}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        return (isGeminiFlow ? data['result'] : data['choices'][0]['message']['content']) as String;
      } else {
        throw Exception('Localhost Error (${response.statusCode}): ${response.body}');
      }
    } catch (e) {
