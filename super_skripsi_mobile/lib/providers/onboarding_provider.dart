...
class OnboardingNotifier extends StateNotifier<OnboardingState> {
  ...
  Future<void> _loadState() async {
    ...
    try {
      ...
      final int lastStepInt = (lastStep is int) ? lastStep : 0;
      
      state = state.copyWith(
        isCompleted: isCompleted && sessionRestored, 
        surveySource: surveySource as String?,
        currentStep: (isCompleted && sessionRestored) ? 2 : lastStepInt,
        googleName: googleName as String?,
        googleEmail: googleEmail as String?,
        googlePhotoUrl: googlePhotoUrl as String?,
        customPhotoPath: customPhotoPath as String?,
        isAuthenticating: false,
      );
    } catch (e) {
      debugPrint('🚨 Cache Onboarding Rusak: $e');
      // Reset state jika cache korup
      state = OnboardingState(isAuthenticating: false);
    }
  }
